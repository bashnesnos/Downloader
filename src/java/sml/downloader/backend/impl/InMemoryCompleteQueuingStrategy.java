 ...
 * Проще по той причине, что нам нужно быстрое обновление статуса -> поиск по requestId - за это отвечает HashMap
 * С другой стороны нам нужна ограниченная concurrent очередь для производителя-потребителя - за это ArrayBlockingQueue
 * Это конечно дорого в плане памяти, но зато быстро и просто
 * 
 * 
